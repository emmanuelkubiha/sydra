...
/**
 * Réponse JSON unique pour éviter les sorties partielles/non-JSON côté frontend.
 */
...
try {
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
        respond_json(['success' => false, 'ok' => false, 'message' => 'Méthode non autorisée'], 405);
    }

    $sessionUserId = (int) ($_SESSION['auth_user_id'] ?? $_SESSION['user_id'] ?? 0);
    if ($sessionUserId <= 0) {
        respond_json(['success' => false, 'ok' => false, 'message' => 'Session expirée'], 401);
    }

    $csrf = (string) ($_POST['csrf'] ?? '');
    if ($csrf === '' || !isset($_SESSION['csrf_token']) || !hash_equals((string) $_SESSION['csrf_token'], $csrf)) {
        respond_json(['success' => false, 'ok' => false, 'message' => 'Token CSRF invalide'], 419);
    }
    ...
    if ($reporterCol === null) {
        throw new RuntimeException('Colonne reporter introuvable.');
    }
    ...
    if ($statusIdExpr) {
        try {
            ...
            // Fallback silencieux vers l'ID mappé si la table de référence n'existe pas.
        }
    }
    ...
    if (!is_array($report)) {
        respond_json(['success' => false, 'ok' => false, 'message' => 'Rapport introuvable'], 404);
    }

    $reporterUserId = (int) ($report['reporter_user_id'] ?? 0);
    if ($reporterUserId <= 0) {
        throw new RuntimeException('Reporter introuvable.');
    }
    ...
    if ($email !== '' && !(bool) ($mailResult['success'] ?? false)) {
        respond_json([
            'success' => true,
            'ok' => true,
            'warning' => 'Décision enregistrée, mais le serveur mail est indisponible. L\'organisation n\'a pas reçu l\'email.',
            'report_id' => $reportId,
            'action' => $action,
            'status' => $target['label'],
    ...
    respond_json([
        'success' => true,
        'ok' => true,
        'message' => 'Décision soumise avec succès et email envoyé.',
        'report_id' => $reportId,
        'action' => $action,
        'status' => $target['label'],
        'server_status' => 'updated',
    ...
    respond_json([
        'success' => false,
        'ok' => false,
        'message' => 'Le serveur n\'a pas pu traiter la décision.',
        'error' => $e->getMessage(),
    ], 500);
} catch (Throwable $e) {
    respond_json([
